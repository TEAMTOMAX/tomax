...
brothers = Token:match("(%d+)")
redis:set(brothers..":VERSION",2.9)
redis:set(brothers..":SUDO_ID:",SUDO_USER)
redis:set(brothers..":DataCenter:",'German')
redis:set(brothers..":UserNameBot:",BOT_User)
redis:set(brothers..":NameBot:",BOT_NAME)
redis:hset(brothers..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3brothers_INSTALL","Yes")
info = {}
...
brothers = Token:match("(%d+)")
our_id = tonumber(brothers)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(brothers..":UserNameBot:")
SUDO_ID = tonumber(redis:get(brothers..":SUDO_ID:"))
SUDO_USER = redis:hgetall(brothers..'username:'..SUDO_ID).username
version = redis:get(brothers..":VERSION")
DataCenter = redis:get(brothers..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..brothers..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(brothers..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(brothers..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(brothers..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(brothers..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'مطور في بوت👷‍♂'
msg.TheRank = 'مطور في بوت👷‍♂'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(brothers..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'منشئ الاساسي👨‍🚀'
msg.TheRank = 'منشئ الاساسي👨‍🚀'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(brothers..':Hussain:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'منشئ الكروب👨‍💻'
msg.TheRank = 'منشئ الكروب👨‍💻'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(brothers..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير👨‍✈️' 
msg.TheRank = 'مدير في بوت👨‍✈️' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(brothers..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'ادمن🕵‍♂'
msg.TheRank = 'ادمن في بوت🕵‍♂'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(brothers..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(brothers..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(brothers..'lock_bots_by_kick'..msg.chat_id_) then
...
if msg.GroupActive and redis:get(brothers..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(brothers..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(brothers..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(brothers..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.brothers then 
if msg.text and ScriptFile.ibrothers then
for k, brothers in pairs(ScriptFile.brothers) do
local SearchText = msg.text:match(brothers)
if SearchText then
if not CheckFlood(msg) then
return false 
end
local GetMsg = ScriptFile.ibrothers(msg,{msg.text:match(brothers)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",brothers.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dbrothers then
if ScriptFile.dbrothers(msg) == false then
return false
end
...
if Plug.brothers then 
if msg.text and Plug.ibrothers then
for k, brothers in pairs(Plug.brothers) do
local SearchText = msg.text:match(brothers)
if SearchText then
if not CheckFlood(msg) then
return false
end
local GetMsg = Plug.ibrothers(msg,{msg.text:match(brothers)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",brothers.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dbrothers then
Plug.dbrothers(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source brothers")
end 

end
else
print("The File Script.lua Not Runing in The Source brothers")
...
function tdcli_update_callback(data)
	...
	redis:srem(brothers..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/BrothersTeam/Brothers/master/inc/Brothers.lua','./inc/Brothers.lua')
...
download(stk,32)
	...
	if redis:get(brothers..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(brothers..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(brothers..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(brothers..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(brothers..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(brothers..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(brothers..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(brothers..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(brothers..'group:add'..data.chat_.id_) then
	redis:set(brothers..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	redis:mset(
	brothers..'group:name'..k,IDS.Title,
	brothers..'num_msg_brothers'..k,5,
	brothers..'group:add'..k,true,
	brothers..'lock_link'..k,true,
	brothers..'lock_spam'..k,true,
	brothers..'lock_webpage'..k,true,
	brothers..'lock_markdown'..k,true,
	brothers..'lock_flood'..k,true,
	brothers..'lock_bots'..k,true,
	brothers..'mute_forward'..k,true,
	brothers..'mute_contact'..k,true,
	brothers..'mute_document'..k,true,
	brothers..'mute_inline'..k,true,
	brothers..'lock_username'..k,true,
	brothers..'replay'..k,true
	)
	redis:sadd(brothers..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(brothers..'username:'..ID,'username',user)
	redis:sadd(brothers..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(brothers..'username:'..ID,'username',user)
	redis:sadd(brothers..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(brothers..'username:'..ID,'username',user)
	redis:sadd(brothers..'owners:'..k,ID)
	end
	end
	end
	...
	redis:srem(brothers..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(brothers..'username:'..data.user_.id_).username
	...
	redis:hset(brothers..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
